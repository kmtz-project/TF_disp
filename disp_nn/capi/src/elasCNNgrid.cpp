...
void compute (const char* file_1, const char* file_2, const char* outfile, int maxdisp, int no_interp, int cosine_weight, 
              int grid_size, np::ndarray GT_arr, np::ndarray conv_left_arr, np::ndarray conv_right_arr) {
    ...
    param.GT = (float *) GT_arr.get_data();
    param.candidate_stepsize = grid_size;
    Elas elas(param);
...
BOOST_PYTHON_MODULE(elasCNNgrid)
{
    np::initialize();
    def("compute", compute);
}
