...
#endif
  computeDisparity(p_support,tri_1,disparity_grid_1,grid_dims,desc1.I_desc,desc2.I_desc,0,D1,conv_left,conv_right);
  computeDisparity(p_support,tri_2,disparity_grid_2,grid_dims,desc1.I_desc,desc2.I_desc,1,D2,conv_left,conv_right);
...
inline int16_t Elas::computeMatchingDisparity (float* conv_left, float* conv_right, const int32_t &u,const int32_t &v,
  ...
  if (u>=window_size+u_step && u<=width-window_size-1-u_step && v>=window_size+v_step && v<=height-window_size-1-v_step) {
    ...
    for (int16_t d=disp_min_valid; d<=disp_max_valid; d++) {
      ...
      const int features = 112;
      float cosine_dist = 0;
      float left_norm = 0;
      float right_norm = 0;
      float sum = 0;
      float* left_row = nullptr; 
      float* right_row = nullptr;

      for (int i = 0; i < features; i++) {
        left_row = get_row(conv_left, v,  u, width, features);
        right_row = get_row(conv_right, v,  u_warp, width, features);
        sum += left_row[i]*right_row[i];
        left_norm += left_row[i]*left_row[i];
        right_norm += right_row[i]*right_row[i];
      }
      if (left_norm > 0 && right_norm > 0) {
        cosine_dist = 1 - sum/(sqrt(left_norm)*sqrt(right_norm));
      }
      else {
        cosine_dist = 1;
      }
      sum = cosine_dist*param.support_cosine_weight;
      //cout << "sum " << sum << endl;
    ...
    //cout << "min_1_E " << min_1_E << " min_2_E " << min_2_E << "\n";
  ...
  for (int32_t u_can=1; u_can<D_can_width; u_can++) {
    ...
    for (int32_t v_can=1; v_can<D_can_height; v_can++) {
      ...
      *(D_can+getAddressOffsetImage(u_can,v_can,D_can_width)) = -1;
      ...
      // find forwards
      d = computeMatchingDisparity(conv_left, conv_right,u,v,I1_desc,I2_desc,false);
      if (d>=0) {
        sup_good++;
        // find backwards
        d2 = computeMatchingDisparity(conv_right, conv_left,u-d,v,I1_desc,I2_desc,true);
...
float* Elas::get_row(float * data, int i,  int j, int column_num, int el_num) {
	return (data + el_num * (i * column_num + j));
}

void Elas::get_CNN_cosine(float* conv_left, float* conv_right, int32_t &u, int32_t &v, int32_t &width,int32_t &u_warp,
					 	              const int32_t &d, int32_t &val,int32_t &min_val,int32_t &min_d) {
	const int features = 112;
	float cosine_dist = 0;
	float left_norm = 0;
	float right_norm = 0;
	...
	val = cosine_dist*param.cosine_weight;
	if (val<min_val) {
    	min_val = val;
    	min_d   = d;
  	}
...
void Elas::get_CNN_cosine(float* conv_left, float* conv_right, int32_t &u, int32_t &v, int32_t &width,int32_t &u_warp, const int32_t &w,
						              const int32_t &d, int32_t &val,int32_t &min_val,int32_t &min_d) {
	const int features = 112;
	const float cosine_weight = 50;
	float cosine_dist = 0;
	float left_norm = 0;
	float right_norm = 0;
	float sum = 0;
	...
	val = cosine_dist*param.cosine_weight+w;
	if (val<min_val) {
    	min_val = val;
    	min_d   = d;
  	} 
...
inline void Elas::findMatch(int32_t &u,int32_t &v,float &plane_a,float &plane_b,float &plane_c,
                            ...
                            int32_t *P,int32_t &plane_radius,bool &valid,bool &right_image,float* D, float* conv_left, float* conv_right){
  ...
  if (!right_image) { 
    for (int32_t i=0; i<num_grid; i++) {
      ...
      if (d_curr<d_plane_min || d_curr>d_plane_max) {
        ...
        get_CNN_cosine(conv_left, conv_right, u, v, width, u_warp,d_curr, val,min_val,min_d);
      }
    }
    for (d_curr=d_plane_min; d_curr<=d_plane_max; d_curr++) {
      ...
      if (u_warp<window_size || u_warp>=width-window_size)
      	continue;
      get_CNN_cosine(conv_left, conv_right, u, v, width, u_warp,valid?*(P+abs(d_curr-d_plane)):0,d_curr, val,min_val,min_d);
  ...
  // right image
    ...
    for (int32_t i=0; i<num_grid; i++) {
      ...
      if (d_curr<d_plane_min || d_curr>d_plane_max) {
        ...
        if (u_warp<window_size || u_warp>=width-window_size)
          continue;
        get_CNN_cosine(conv_right, conv_left, u, v, width, u_warp,d_curr, val,min_val,min_d);
        //updatePosteriorMinimum((__m128i*)(I2_line_addr+16*u_warp),d_curr,xmm1,xmm2,val,min_val,min_d);
      }
    }
    for (d_curr=d_plane_min; d_curr<=d_plane_max; d_curr++) {
      ...
      if (u_warp<window_size || u_warp>=width-window_size)
        continue;
      get_CNN_cosine(conv_right, conv_left, u, v, width, u_warp,valid?*(P+abs(d_curr-d_plane)):0,d_curr, val,min_val,min_d);
      //updatePosteriorMinimum((__m128i*)(I2_line_addr+16*u_warp),d_curr,valid?*(P+abs(d_curr-d_plane)):0,xmm1,xmm2,val,min_val,min_d);
    }
  }
...
void Elas::computeDisparity(vector<support_pt> p_support,vector<triangle> tri,int32_t* disparity_grid,int32_t *grid_dims,
                            uint8_t* I1_desc,uint8_t* I2_desc,bool right_image,float* D, float* conv_left, float* conv_right) {
  int valid_num = 0;
          ...
          for (int32_t v=min(v_1,v_2); v<max(v_1,v_2); v++)
            if (!param.subsampling || v%2==0) {
              findMatch(u,v,plane_a,plane_b,plane_c,disparity_grid,grid_dims,
                        I1_desc,I2_desc,P,plane_radius,valid,right_image,D,conv_left,conv_right);
            }
        }
      }
    }
  ...
  if (param.std_filter) {
  	for (int32_t u=0; u<D_width; u++) {
    	for (int32_t v=0; v<D_height; v++) {
      		...
      		float* mask_row = get_row(param.mask, v,  u, D_width, 1);
      		if (mask_row[0] > 0) {
        		*(D+addr_start) = (int) mask_row[0]*param.disp_max/255;
      		}
    	}
  	}
  }
