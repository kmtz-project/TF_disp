...
right input patches and outputs) used for training. It also considers
occlusion mask.
'''
def get_batch_occ(folder_name, patch_size, neg_low, neg_high, scale):
    ...
    for i in range(0, height):
        for j in range(0, width):
            if occ_pix[i, j] == 255 and j >= (int(disp0_pix[i, j]/scale) + int(patch_size/2)) and j < (width - int(patch_size/2)):
