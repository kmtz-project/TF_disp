...
#samples_list   =  ["Adirondack", "ArtL", "Motorcycle", "Piano", "Recycle", "Shelves", "Teddy", "snowflake1", "cones"]
...
#handlers_list   = ["convFullNN", "convFastNN", "opencvSGBM", "ELAS", "ELAS-CNN", "ELAS-CNN-sup", "ELAS-CNN-grid"]#, "ELAS-CNN-fusion", "ELAS-CNN-std"]
handlers_list   = ["opencvSGBM","ELAS","ELAS-CNN","ELAS-CNN-sup"]
...
for sample_name in samples_list:
    ...
    golden_disp0_fname = samples_fname + sample_name + "/disp0.png"
    ...
    for handler_name in handlers_list:
        ...
        exists = os.path.isfile(golden_disp_fname)
        if exists:
            golden_disp = pfm.readPfm(golden_disp_fname)
        else:
            golden_disp = Image.open(golden_disp0_fname).convert("L")
            golden_disp = numpy.atleast_1d(golden_disp).astype('int')
